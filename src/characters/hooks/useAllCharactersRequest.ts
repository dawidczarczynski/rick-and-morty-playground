...
function fetchCharacters({ pageParam, queryKey }: QueryFunctionContext<[ string, CharactersParams], string>) {
    ...
    const requestParams = !pageParam ? queryParams as unknown as ApiRequestParams : {};

    return makeRequest<Character[]>(requestURL, requestParams);
}

export function useAllCharactersRequest(params: CharactersParams) {
   return useInfiniteQuery<ApiResponse<Character[]>, string, ApiResponse<Character[]>, [string, CharactersParams]> (
        [CHARACTERS_RESOURCE_NAME, params],
        fetchCharacters,
        { 
